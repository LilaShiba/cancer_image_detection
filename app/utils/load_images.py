    ...
    def main_loop(self) -> dict:
        ...
        for idx, (fname, img_path, category) in enumerate(tqdm(self.image_paths, desc="Processing images")):
            ...
            try:
                ...
                if random.randint(0, 1) >= 0.90:
                    random_choice_method = random.choice([
                    ...
                    processed_images.append(random_choice_method)
                ...
                processed_images_list.extend(
                    (category, img) for img in processed_images)

            except Exception as e:
                print(f"Error processing {img_path}: {e}")

        # Parse each image into DataFrame as columns: (Label, Numpy Array)
        self.df = pd.DataFrame(processed_images_list,
                               columns=['Label', 'Image'])
