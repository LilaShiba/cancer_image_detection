...
class Convoultion_NN(ImageLoad):
    def __init__(self, dataset_path: str, learning_rate: float = 0.001, batch_size: int = 32,
        ...
        self.df['EncodedLabel'] = self.label_encoder.fit_transform(
            self.df['Label'])
    ...
    def create_image_tensors(self) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        label_tensors = torch.tensor(
            self.df['EncodedLabel'].values, dtype=torch.long).to(self.device)
    ...
    def build_model(self, architecture: str = "deep-wide") -> nn.Sequential:
        """
        Builds the CNN model based on the specified architecture for cancer cell image recognition.
        ...
        if architecture == "deep-wide":
            # Wide and deep architecture for complex feature extraction
            layers = [
                # Input: 128 x 128 x input_channels

                # Convolution Block 1
                nn.Conv2d(self.input_channels, 64, kernel_size=3,
                          stride=1, padding=1),  # Output: 64 x 128 x 128
                nn.BatchNorm2d(64),
                ...
                nn.Conv2d(64, 128, kernel_size=3, stride=1,
                          padding=1),  # Output: 128 x 64 x 64
                nn.BatchNorm2d(128),
                nn.LeakyReLU(negative_slope=0.01),
                ...
                nn.Conv2d(128, 256, kernel_size=3, stride=1,
                          padding=1),  # Output: 256 x 32 x 32
                nn.BatchNorm2d(256),
                nn.LeakyReLU(negative_slope=0.01),
                ...
                nn.Conv2d(256, 512, kernel_size=3, stride=1,
                          padding=1),  # Output: 512 x 16 x 16
                nn.BatchNorm2d(512),
                nn.LeakyReLU(negative_slope=0.01),
                ...
                nn.AdaptiveAvgPool2d(1),  # Output: 512 x 1 x 1
    ...
    def train_model(self, epochs: int):
        ...
        for epoch in range(epochs):
            ...
            for img_tensors, label_tensors in tqdm(train_dataloader, total=len(train_dataloader)):
                img_tensors, label_tensors = img_tensors.to(
                    self.device), label_tensors.to(self.device)
    ...
    def process_image(self, file_path: str) -> str:
        ...
        with torch.no_grad():  # Disable gradient calculation for inference
            img_tensor = self.image_to_tensor(
                file_path)  # Convert image to tensor
            ...
            predicted_label = self.label_encoder.inverse_transform(
                [predicted_label_index])[0]  # Convert back to label
    ...
    def test_model(self, test_image_tensors: torch.Tensor = None, test_label_tensors: torch.Tensor = None) -> float:
        ...
        with torch.no_grad():  # Disable gradient computation for testing
            for i in tqdm(range(total)):
                img_tensor = self.image_tensors[i].unsqueeze(
